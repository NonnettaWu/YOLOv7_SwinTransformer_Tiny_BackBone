...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # assert H.tolist() == self.img_size[0] and W.tolist() == self.img_size[1], \
        #     f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]} * {self.img_size[1]})."
        #-------------------------------------------------------#
        #   bs, 3, 640, 640 -> bs, 96, 160, 160 -> bs, 25600, 96
        #-------------------------------------------------------#
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        #-----------------------------------------------#
        #   bs, 25600, 96 -> bs, 160, 160, 96
        #-----------------------------------------------#
        shortcut = x
        x = self.norm1(x)       # [B, 25600, 96]
        x = x.view(B, H, W, C)  # [B, 160, 160, 96]
        ...
        if self.shift_size > 0:
            ...
            shifted_x = x
        #------------------------------------------------------------------------------------------#
        #   bs, 160, 160, 96 -> bs * 64, 7, 7, 96 -> bs * 64, 49, 96
        #------------------------------------------------------------------------------------------#
...
def Swin_transformer_Tiny(pretrained = False, input_shape = [224, 224], **kwargs):
